...
import { writeFileSync, readFileSync, existsSync, statSync, createReadStream, createWriteStream } from 'fs';

const pipe = promisify(pipeline);
function index ({ entryPoint = '.svelte-kit/node/index.js', out = 'build', precompress, env: { path: path_env = 'SOCKET_PATH', host: host_env = 'HOST', port: port_env = 'PORT' } = {}, esbuild: esbuild_config } = {}) {
    return {
                ...
                plugins: [
                    {
                        ...
                        setup(build) {
                            ...
                            build.onResolve({ filter: /\/middlewares\.js$/ }, ({ path, resolveDir }) => {
                                ...
                                if (resolved === internal_middlewares_path ||
