	...
	return {
		...
		async adapt({ utils, config }) {
			...
			writeFileSync(
				...
				process.env.__KIT_IMAGE_OPTS = ${JSON.stringify(imageConfig)};
				//import.meta.env.__KIT_IMAGE_OPTS = ${JSON.stringify(imageConfig)};

				export const imageConfig = ${JSON.stringify(imageConfig)}`
			);
